...
normalized_y = librosa.util.normalize(data)

# Compute STFT
D = librosa.core.stft(normalized_y, n_fft=n_fft, hop_length=hop_length)
...
mel = librosa.feature.melspectrogram(S=D, n_mels=n_mels)
...
print('normalizate.shape',normalized_mel.shape)
...
for index,y in enumerate(mylist):
	if mylist[index][6:-16]!='01' and mylist[index][6:-16]!='07' and mylist[index][6:-16]!='08' and mylist[index][:2]!='su' and                mylist[index][:1]!='n' and mylist[index][:1]!='d':
           ...
           # STFT
           normalized_y = librosa.util.normalize(X)
           # Compute STFT
           D = librosa.core.stft(normalized_y, n_fft=n_fft, hop_length=hop_length)
...
def make_dilated_network(num_rows, num_columns, num_channels):
    ...
    x = Conv2D(filters=256, kernel_size=5)(input_data)
    x = Conv2D(filters=256, kernel_size=5, padding = 'same')(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)

    x_1 = Conv2D(filters=256, kernel_size=5, padding = 'same')(x)
    x_1 = MaxPooling2D(pool_size=2)(x_1)
    x_1 = BatchNormalization()(x_1)
    x_2 = Conv2D(filters=256, kernel_size=5, padding="same", dilation_rate=2)(x)
    x_2 = MaxPooling2D(pool_size=2)(x_2)
    x_2 = BatchNormalization()(x_2)
    x = Add()([x_1, x_2])
    x = LeakyReLU()(x)
    
    x_1 = Conv2D(filters=128, kernel_size=5, padding = 'same')(x)
    x_1 = MaxPooling2D(pool_size=2)(x_1)
    x_1 = BatchNormalization()(x_1)
    x_2 = Conv2D(filters=128, kernel_size=5, padding="same", dilation_rate=2)(x)
    x_2 = MaxPooling2D(pool_size=2)(x_2)
    x_2 = BatchNormalization()(x_2)
    x = Add()([x_1, x_2])
    x = LeakyReLU()(x)
...
#rms = RMSprop(lr=0.005, rho=0.9, epsilon=None, decay=0.0)
...
history = model.fit(x_train, y_train, batch_size=16, epochs=700, validation_data=(x_test,y_test),callbacks=[csv_logger])
...
target_names=['calm', 'happy', 'sad', 'angry', 'fearful']
