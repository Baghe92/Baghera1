...
def make_dilated_network(num_rows, num_columns, num_channels):
    ...
    x = Conv2D(filters=16, kernel_size=5, padding = 'same')(input_data)
    x = LeakyReLU(alpha=leaky_relu_alpha)(x)
    x_1 = Conv2D(filters=32, kernel_size=3, padding = 'same')(x)
    x_2 = Conv2D(filters=32, kernel_size=3, padding="same", dilation_rate=2)(x)
    x = Concatenate()([x_1, x_2])
    x = LeakyReLU(alpha=leaky_relu_alpha)(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = BatchNormalization()(x)
    x_1 = Conv2D(filters=64, kernel_size=3, padding = 'same')(x)
    x_2 = Conv2D(filters=64, kernel_size=3, padding="same", dilation_rate=2)(x)
    x = Concatenate()([x_1, x_2])
    x = LeakyReLU(alpha=leaky_relu_alpha)(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = BatchNormalization()(x)
...
cnnhistory=model.fit(x_train, y_train, batch_size=64, epochs=700, validation_data=(x_test, y_test),callbacks=[csv_logger])
